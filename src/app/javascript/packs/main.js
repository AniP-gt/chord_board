...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for(let i = 0; i < array.length; i++) {
     // コード一覧
     chord_name+="<div id="+array[i]+" class='chord'><a onclick="+array[i]+"();add('"+array[i]+"')><img src='/img/"+array[i]+".png' class='chord_img' alt="+array[i]+"></a></div>";
     // 音源ファイル
     ongen+="<audio id=\"sound-file_"+array[i]+"\" preaudio=\"auto\"><source src=\"/music/"+array[i]+",.mp3\" type=\"audio/mp3\"></audio>";
     // コードを表示
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for(let i = 0; i < array.length; i++) {
     // ディグリーネーム一覧
     degree_name+="<div id="+array[i]+" class='degree'><a onclick=add('"+array[i]+"')><img src='/img/"+array[i]+".png' class='degree_img' alt="+array[i]+"></a></div>";
     // ディグリーネームを表示
...
function add(a){
   ...
   for (let i = 0; i < view.length; i++) {
      ...
      console.log(str);
   // クリック後に表示される要素
    chord_list+="<div id="+str+" class='chord'><a class='sakuhin'><img src='/img/"+str+".png' class='chord_img' alt="+str+"></a><p class='remove' onClick=\"sakuzyo("+i+")\">×削除</p></div>";
   //  $form_hidden+="@"+str+"$,";
...
function sakuzyo(S){
   ...
   //クッキーの振りなおし
     if(i!=S){
     $sakuhin_botan+="<div id="+str+" class='chord'><a class='sakuhin'><img src='/img/"+str+".png' class='chord_img' alt="+str+"></a><p class='remove' onClick=\"sakuzyo('"+i+"')\">×削除</p></div>\n";
     $furinaosi+= str + ",";
     };
     // クッキーの再設定
