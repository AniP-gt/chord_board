...
window.addEventListener('DOMContentLoaded', () => {
  ...
  for(let i = 0; i < array.length; i++) {
     // コード一覧
    chord_name+="<div id="+array[i]+" class='chord'><a onclick="+array[i]+"();add('"+array[i]+"')><img src='/assets/"+array[i]+".png' class='chord_img' alt="+array[i]+"></a></div>";
     // 音源ファイル
...
function add(a){
  ...
  for (let i = 0; i < view.length; i++) {
      ...
      console.log(str);
   // クリック後に表示される要素
    chord_list+="<div id="+str+" class='chord'><a class='sakuhin'><img src='/assets/"+str+".png' class='chord_img' alt="+str+"></a><p class='remove' onClick=\"sakuzyo("+i+")\">×削除</p></div>";
   //  $form_hidden+="@"+str+"$,";
...
function sakuzyo(S){
  ...
  for (let i = 0; i < view.length; i++) {
   ...
   //クッキーの振りなおし
    if(i!=S){
    $sakuhin_botan+="<div id="+str+" class='chord'><a class='sakuhin'><img src='/assets/"+str+".png' class='chord_img' alt="+str+"></a><p class='remove' onClick=\"sakuzyo('"+i+"')\">×削除</p></div>\n";
    $furinaosi+= str + ",";
    };
     // クッキーの再設定
